...
def build_default_module_registry() -> ModuleRegistry:
    ...
    reg.register(ModuleManifestV2(
        module_id="workflow", name="工作流与 Agent", version="1.0.0",
        domain="workflow", status="live", theme_token="purple",
        primary_route="/workflow",
        navigation=(
            NavRoute(route="/workflow/runs", label="Graph Runs",
                     description="Graph+Loop 运行与 trail",
                     actions=("查看 trail", "checkpoint", "恢复")),
            NavRoute(route="/workflow/agents", label="Agent 矩阵",
                     description="Agent Manifest、权限与健康",
