...
SCOPES = (
    ...
    # R2-03：认知内核 / Research RAG 权限（fail-closed，未知 scope 拒绝）
    "cognition.read", "cognition.manage",
    "research.read", "research.run", "research.decide",
    # 统一模型管理 V1（M5/G4，02 §7）：普通员工默认无任何 models scope
...
BUILTIN_ROLES: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    ...
    # 统一模型管理 V1（M5/G4）：受限角色矩阵
    # 模型管理员：可创建/测试/轮换/提交，无批准权（maker）
    ("model_admin", "模型管理员", ("models.use", "models.config.read",
                                 "models.connection.manage",
                                 "models.secret.rotate",
                                 "models.binding.manage",
                                 "models.usage.read")),
    # 模型审批人：批准他人提交（checker），不持有 secret 轮换权
    ("model_approver", "模型审批人", ("models.config.read",
                                    "models.release.approve",
                                    "models.usage.read")),
)
