...
- P1：Workflow 仅运行查看器；Manifest 与 Capability/Route 不是真正一体化；
  主管 Agent 不是工具规划执行器（Phase C/Z 关闭）；
