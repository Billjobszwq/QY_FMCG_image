...
| ABOSV2-P1-002 | P1 | CLOSED_WITH_EVIDENCE | Manifest 与 Capability 两套注册表 | `project()` 未投影执行契约；10 模块仅 4 capability | commit `db1692a4`：投影补 commands/queries/events；集成交叉验证 fail-closed（ok=true） |
| ABOSV2-P1-003 | P1 | CLOSED_WITH_EVIDENCE | App.tsx 手写路由，模块不可真正插拔 | Registry 只驱动导航不驱动页面装载 | commit `db1692a4`：ModuleUIRegistry 统一驱动；三方一致性 14 项契约测试 |
| ABOSV2-P1-004 | P1 | PARTIALLY_CLOSED | 主管 Agent 是关键词路由器 | supervisor.py if/elif；delegation 只是回执 | Workflow Agent draft 受控 + 六 Agent 身份/allowlist 注册；独立对话运行时待后续 |
