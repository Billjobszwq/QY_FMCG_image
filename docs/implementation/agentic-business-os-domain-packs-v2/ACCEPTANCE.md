...
## Gate G6 · BI（已通过，2026-08-12）

- [x] Agent 不能生成越权查询（未注册指标 fail-closed，无任意 SQL；测试+现场）
- [x] 同一数字可下钻：报表值 → 事实表（usage/survey）；project 维度拆分
- [x] 异常 → 追问 WorkItem → 回答 → 报告刷新新版本（旧报告不覆盖）
- [x] 发布人工批准门（现场 409 拦截）

## Gate G7 · 位置外勤（已通过，2026-08-12）

- [x] 路线约束、未分配原因、费用可解释（硬隔离/max_km 现场证据）
- [x] 低置信度地址不自动派发（现场 409）；位置保留策略字段存在
- [x] 到店围栏半径+精度校验；门头必拍；人脸默认不自动触发（现场 409）
- [x] 全链：任务→路线→到店→问卷/照片→差旅费（ft-f6b4b52ab4eb，20.94 元）

## Gate G8 · 财务（已通过，2026-08-12）

- [x] 所有账单行可追溯到 immutable usage 与业务 run（inv-02275a438bb9 下钻）
- [x] 历史价格不被新价格覆盖（rate card v2 后已开票 total 不变）
