...
| 15 | VLM-015：红测试 `tests/platform/test_new_packaging.py`（模块不存在 collection error RED）→ 实现 → 18 项全绿 | 0 | 迁移 017 追加式：package_decision 台账（六态 CHECK+source CHECK）+ package_supersede 追加表 + 两枚不可变触发器（禁 DELETE、终态禁 UPDATE）；`packaging.py`：candidate→reviewing→终态四选一，创建仅限 candidate（Qwen 直产终态拒绝），终结仅限 human/customer_policy；名称选择 keep_old_name/adopt_new_name/create_new_sku 三分支，显示名与 package_version 分离，名称变化绝不改 sku_id；终态双保险拒更新拒删除（应用层+触发器）；supersede 只追加不改写旧行（测试逐字段相等校验），双方必须已终结，重复拒绝；终结写审计 packaging.finalized；store.py 增六个类型化方法；真实训练未受影响，零 MPS/网络动作；全量 **739 passed + 1 skipped**（721+18） |
| 16 | VLM-016：红测试 `tests/platform/test_cascade_web_contract.py`（页面不存在 + create_cascade_router 无 packaging 参数，5 failed + 9 errors RED）→ 实现 → 14 项全绿 | 0 | 静态 UI 合约：CascadeTasks/ModelRuntime/NewPackaging 必须含业务语言（客户档位/当前阶段/为何升级/待人工/剩余 SLA/成本/证据），技术字段（模型哈希/策略版本/risk/token）details 折叠；App.tsx 三路由三导航，api.ts 新增 cascade/models-runtime/packaging 客户端函数；API 行为合约：models/runtime 空时诚实 {count:0,models:[]}；任务详情补 billing 账本与 remaining_sla（CascadeService.sla_hours，12/48h 队列业务 SLA）；packaging 裁决端点由 facade 注入（平台不反向 import Domain Pack）：finalize 来源固定 human 进审计、非法终态 400、沿用旧名不改 sku_id 但生成新 package_version、未终结不得 supersede；识别/GraphRuns/训练页只加跳转摘要不复制功能；`npm run build` 成功（45 modules）；8400 真实浏览器验证三页面无白屏无 console error，级联 API 未启用/未登录/空队列均诚实降级；parseError 统一带 HTTP 状态码；旧 8091/8092 零影响，真实训练未受影响，零 MPS/网络重动作；全量 **753 passed + 1 skipped**（739+14） |
| 17 | VLM-017：红测试 `tests/unit/test_cascade_shadow_eval.py`（模块不存在 collection error RED）→ 实现 → 27 项全绿 | 0 | 新建 `src/eval/cascade_shadow.py`：truebox one-to-one 匹配（IoU 降序贪心，GT/pred 各至多一次）；逐实例账本 9 类标签（correct_accepted/misclassification/duplicate_box/background_fp/missed_truth/abstain/unknown/new_package/manual_review）+ latency/cost 入账；accepted_precision 与 coverage 必须同时报告（coverage=已配对人工真值/人工真值总数，含 abstain 配对）；define_arms 四臂 E0/E1/C1/C2 共享同一 frozen_data_hash/registry_hash/region matching（C2=qwen3-vl:4b，E1 不发布），validate_arms 拒绝跨配置对比；promotion_gate：precision≥0.95 且 coverage≥0.90 且 FP/photo、p95、成本、人工率、unknown-new 不超批准线，人工真值<20 → not_evaluable，绝不造 pass；shadow_execution_gate：进程表 src.training.train_v1/mlx_vlm.lora 或活跃租约 → BLOCKED_BY_ACTIVE_TRAINING fail-closed；新建 `scripts/run_cascade_shadow_eval.py`：evaluate 模式纯计算（可与训练并行），run 模式三级 fail-closed（exit 2 训练阻断/3 未授权/4 G-APPLE 未通过 MLX/Qwen 未安装）；`docs/experiments/qwen3vl-cascade-shadow-report.md` 诚实声明：真实 shadow 未运行、无任何数字指标、不得据此晋级；训练 epoch 39 正常（CPU 22% 无抢占）；零 MPS/网络重动作；全量 **780 passed + 1 skipped**（753+27） |
| 18 | VLM-018：文档、状态和最终交付（无红测试，纯文档+回归） | 0 | docs/README.md 增 runbook 入口与 2026-08-01 验收事实补充；STATUS.md 增 Task 0–18 验收事实表（状态语义 implemented/tested 与 benchmarked/trained/shadow_passed/publish_approved/production_active 严格区分）；ISSUES.md 全部 OPEN 项保留，新增 VLM-ISSUE-007/008；DECISIONS.md 新增 PV2-D-018～021；新建 `docs/runbooks/qwen3vl-cascade-local-runbook.md`：环境/preflight/数据集/zero-shot/benchmark/pilot/shadow/驻留/故障排查，5 个 CLI（run_qwen3vl_{preflight,zero_shot,benchmark,lora}/build_qwen3vl_dataset/run_cascade_shadow_eval）全部 --help parse-only 验证参数真实存在；未关闭项（人工 truebox/质量金标准/934 tilt/新包装金标准/生产发布）保持 OPEN；最终回归全量 **780 passed + 1 skipped**（20.5s，1 skipped 为既有真实模型相关跳过不影响门禁）；git status 只余计划内修改 + 保留未跟踪制品（.quality/.sam_checkpoints/.sam_runs/.superpowers）；git diff --check 无 whitespace error；production bundle 未切换，训练 epoch 39 未受影响；VLM-000～018 全部 DONE |
| 19 | G-CURRENT 对账：sku_v7_sam 训练已自然结束（EarlyStopping，无需人工停止） | 0 | 事实：训练在 epoch 50/120 因 patience=10 早停优雅退出（日志 L404：Best results observed at epoch 40），无任何 kill/中断动作；制品对账：results.csv 50 epoch 完整，best_mAP50=0.6285@ep44，最终 val precision=0.5797/recall=0.6119/mAP50=0.6285/mAP50-95=0.3994；weights/best.pt+last.pt 完整（best.pt sha256 前 16 位 51272c8177809a1b，43M）；目录 97M 未触碰；已知偏差如实保留：optimizer=auto 导致 lr0=0.0005 被忽略（VLM-ISSUE-002）、最终评估与晋级仍为 RUNNING_EXPERIMENTAL→ENDED_EXPERIMENTAL，未判定可发布；资源释放：无训练进程/caffeinate，内存空闲 90%；G-CURRENT 通过 → 用户已授权启动下一阶段（G-APPLE：隔离环境安装 mlx-vlm + 权重下载） |
| 20 | G-APPLE 下一阶段执行：隔离环境安装 + 真实 preflight 通过 | 0 | 事实：.venv_mlx_vlm 安装 mlx-vlm 0.6.10（已入 .gitignore）；权重 mlx-community/Qwen3-VL-4B-Instruct-4bit 下载完成；scripts/run_qwen3vl_preflight.py 升级为真实探针（mlx_metal_device/model_loadable/processor_image/bounded_forward，fail-closed 语义不变）+ 服务健康路径修正为 8091/v2/health、8092/api/live；真实运行 --authorized 结果 run=20260807_125229：14/14 全部通过（Metal GPU、模型加载、processor 处理 64x64 图像、bounded forward max_tokens=8 输出正常、AC 电源、磁盘/内存/swap、服务健康）；G-APPLE 真实通过；全量回归 780 passed + 1 skipped（16.25s）；VLM-ISSUE-005 关闭；production bundle 未切换，8091/8092 口径不变 |
| 21 | zero-shot 与 benchmark 真实执行（隔离环境） | 0 | 事实：新增 scripts/run_qwen3vl_zero_shot_infer.py（闭集重排协议：gt+K-1 确定序干扰项 hash 打乱，只输出编号或 none，fail-closed）与 scripts/run_qwen3vl_benchmark_real.py（注入真实 executor 进 run_benchmark 冻结矩阵）；首次 zero-shot 运行因 mlx-vlm 0.6.10 generate 返回 GenerationResult（非 dict）全 error，修复后重跑 run=20260807_130724：48 crop、coverage=95.8%、accepted_precision=47.8%、top5=100%、escape=0、p95=358ms、wall=14s，gate_pass=false（precision<0.9，如实记录不伪造）；benchmark run=20260807_131457：12 探针实测（batch 1/2/4 × 两档视觉 token × qlora/bf16 标签），estimation_basis=measured，总 wall=8.4s；QLoRA pilot/shadow 如实标记阻断（human_final 真值不足）；全量回归 780 passed + 1 skipped（17.01s）；swap 7172MB 低于 8192 停止线；production bundle 未切换 |
| 22 | Label Studio 可见 SKU 修复 + 历史回填 + 浏览器验收 + 区域级 gold 扩展 | 0 | 根因：`predictions_from_recognition()` 丢弃 sku_id/sku_name/status/margin，只出 rectanglelabels；红测试 10 failed 复现后修复（ac1bb4a：accepted+Registry 映射成立才出 taxonomy，rejected/unknown/越界不伪造，同 region id/同坐标/from_name=box+sku/to_name=image）；LS 1.23 顶层 meta 不保存 → 改挂 region result meta 键（678feb3）；unreviewed 初始状态贯通 label_config/exporter/webhook/状态机（a3b5265），禁止自动写 matched；真实 LS 项目 10/11/12/13 config 补 unreviewed（备份+对账）；Web 页 assisted/blind 入口区分（c77a2f1）；历史回填 append-only：`@visible-sku-v2` 新版本 prediction，项目 10/12 各 +17 区域，errors=0，幂等复扫验证，blind 项目永不回填（be47098）；真实浏览器 12 项验收全过（6 截图，控制台无 error）；区域级 gold：追加式迁移 018 gold_region_v1（no-delete/no-update 触发器）+ submit_review regions fail-closed + 双审一致→human_final/分歧→conflict→仓裁 gold_verified + unknown/new_packaging 弃权 + GET /api/v1/review/gold-summary；250 条审核队列幂等导入生产库；全量 819 passed + 1 skipped |
| 23 | zero-shot 采样与候选检索评估方法修正（V2） | 0 | V1 三处方法缺陷修正：①采样"排序取前 N"（48 crop 只来自 2 张照片）→ 按门店/session/照片/SKU 分层轮转（实采 24 区域覆盖 24 照片/20 门店/11 session/19 SKU）；②候选主动注入 GT → 真实检索链路（crop→OCR 文本→Qwen3-Embedding→KB 余弦检索），retrieve_candidates 签名结构上不接受 GT 参数；③top5 检查完整 K=8 列表 → recall@1/5/8 只取真实 predicted ranking 前 k，候选不足不伪造；evaluate_records_v2 新增 registry_escape/kb_coverage/abstain_rate/每区域成本；真实基线 run=20260807_172812：recall@1/5/8=0、accepted_precision=0、abstain 66.7%、registry_escape 22/24（KB 仅 27 SKU vs 样本 208 类）——诚实暴露 V1 高分系 GT 注入假象；证据级别 sam_refined_interim（正式报告待 human_final≥200 后重跑）；全量 819 passed + 1 skipped；QLoRA 仍未启动 |
