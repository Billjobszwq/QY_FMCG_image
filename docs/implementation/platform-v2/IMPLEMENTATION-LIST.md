...
| U1-006 | MPS G0 真实门禁：arm64/torch MPS built+available/1024²矩阵/模型前向/无 fallback/电源/内存/swap/磁盘，证据写 run | U1-003 | agent | DONE | `tests/platform/test_umt005_mps_g0.py`（7 项含主机门控） | `mps_gate.run_mps_g0` 全项实测；dry_run 写入 mps_g0_report；start_training 对 G0 失败 fail-closed；主机 AC+MPS 实测全绿 | 本批次提交 | 全量 339 passed + 1 skipped |
