...
| VLM-ISSUE-009 | OPEN | 高 | zero-shot 闭集重排 accepted_precision=47.8%（run=20260807_130724，K=8，48 crop），低于 0.9 晋级线，gate_pass=false；说明 4bit zero-shot 无法直接承担闭集裁决，需 QLoRA 微调或检索增强的候选构造，而 QLoRA 被 G-DATA（human_final 真值不足）阻断 | 处置路径：补人工审核真值 → build_qwen3vl_dataset → run_qwen3vl_lora pilot；在此之前 Qwen 级联分支（C2）不得晋级，shadow 维持 not_evaluable |
| VLM-ISSUE-010 | CLOSED | 高 | zero-shot V1 方法缺陷：候选主动注入 GT（gt+干扰项）、采样只来自 2 张照片、top5 检查完整 K=8 列表，导致 top5=100% 等虚高指标 | 已关闭（d44332d）：V2 分层采样 + 真实检索候选（OCR→KB，签名禁 GT）+ recall@1/5/8 取真实 ranking 前 k；V1 证据保留不删除，仅作历史对照 |
| VLM-ISSUE-011 | OPEN | 高 | V2 真实基线（run=20260807_172812）：KB 仅覆盖 27 SKU（搭建初期 P1），分层采样 24 区域中 22 个 GT 不在 KB（registry_escape），candidate recall@1/5/8 均为 0，accepted_precision=0，gate_pass=false | 处置路径：扩 KB/Registry 覆盖（结合 human_final 审核中暴露的 SKU）或用更大候选源；正式报告须待 human_final≥200 区域后以 gold 真值重跑；不得据此判定模型能力（瓶颈在候选源而非模型） |
| VLM-ISSUE-012 | OPEN | 中 | sam_refined_full_v1 val split 834 照片中 362 张无门店/session 元数据（不在 batch2 manifest），分层采样 fail-closed 排除 | 处置：元数据补齐后自动纳入采样；当前 472 张合格照片已足够第一阶段采样 |
