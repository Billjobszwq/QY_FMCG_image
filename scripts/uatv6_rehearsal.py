...
用法：python3 scripts/uatv6_rehearsal.py
"""
...
NS = "uatv6_" + datetime.now().strftime("%Y%m%d%H%M%S") + "_" + \
...
def main() -> int:
    ...
    # ---- 3) 六角色 + 跨客户 403（SI4：创建即携带 test_run 登记
    #      provenance，归档后登录必须被拒绝） ----
    roles = {"pm": "project_manager", "fw": "field_manager",
             "sd": "survey_designer", "an": "analyst",
             "fin": "finance_operator", "aud": "read_only"}
    ...
    for short, role in roles.items():
        ...
        pr = bill.post("/api/v1/iam/principals",
                       {"kind": "user", "username": uname,
                        "display_name": f"UAT V6 {short}",
                        "password": USER_PW, "test_run_id": NS})
        principal_ids.append((pr.get("principal") or {})
                             .get("principal_id", uname))
        ...
        sessions[short] = s
    check("six_roles_login", len(sessions) == 6
          and all(s.csrf for s in sessions.values())
          and bool(bill.csrf), "owner+6 roles")
    fw = sessions["fw"]
    pm = sessions["pm"]
    ...
    check("cross_customer_403_pm", cross_pm.get("_http") == 403,
          str(cross_pm)[:100])
    # 越权写：auditor(read_only) 不得创建客户（写操作 403）
    aud_write = sessions["aud"].post("/api/v1/master/customers",
                                     {"customer_id": f"{NS}_x",
                                      "name": "x"})
    check("priv_write_403", aud_write.get("_http") == 403,
          str(aud_write)[:100])
    ...
    # ---- 8b) SI4 新增：BI 注册表 provenance + Finance dry-run ----
    met = bill.post("/api/v1/analytics/metrics/computed",
                    {"metric_id": f"{NS}_rate", "name":
                     f"UAT V6 自检率 {NS}",
                     "formula": "recognition.tasks * 1",
                     "test_run_id": NS})
    IDS["metric"] = f"{NS}_rate" if met.get("_http") not in (409,) \
        else ""
    met_row = conn.execute(
        "SELECT data_scope, test_run_id FROM bi_metric_v1 WHERE"
        " metric_id=?", (f"{NS}_rate",)).fetchone()
    check("fixture_metric_scoped", met_row is not None
          and met_row["data_scope"] == "uat_fixture"
          and met_row["test_run_id"] == NS,
          dict(met_row) if met_row else met)
    op_metrics = bill.get("/api/v1/analytics/metrics").get(
        "metrics", [])
    check("operational_metric_catalog_clean",
          all(m["metric_id"] != f"{NS}_rate" for m in op_metrics),
          f"visible={len(op_metrics)}")
    dash = bill.post("/api/v1/analytics/dashboards",
                     {"name": f"UAT V6 看板 {NS}",
                      "customer_id": CUST, "widgets": [],
                      "filters": {}, "test_run_id": NS})
    IDS["dashboard"] = dash.get("dashboard_id", "")
    op_dashs = bill.get("/api/v1/analytics/dashboards").get(
        "dashboards", [])
    check("operational_dashboard_list_clean",
          all(d.get("dashboard_id") != IDS["dashboard"]
              for d in op_dashs), f"visible={len(op_dashs)}")
    # data-products 不受 fixture 影响（与运营客户 API 对账）
    dp = bill.get("/api/v1/analytics/data-products")
    dp_cust = next((p["rows"] for p in dp.get("products", [])
                    if p["product"] == "master.customers_v1"), -1)
    op_cust_count = bill.get("/api/v1/master/customers").get("count", -2)
    check("data_products_reconciled",
          dp_cust == op_cust_count,
          f"dp={dp_cust} api={op_cust_count}")
    # Finance dry-run：fixture 客户账单试算（effective usage 已排除，
    # 金额必须为 0，且 invoice 携带 fixture provenance）
    dry = bill.post("/api/v1/finance/invoices/generate",
                    {"customer_id": CUST, "period":
                     datetime.now(timezone.utc).strftime("%Y-%m"),
                     "include_subscription": False})
    inv = dry.get("invoice") or {}
    IDS["finance_dry_run_invoice"] = inv.get("invoice_id", "")
    check("finance_fixture_dry_run_isolated",
          "_http" not in dry and float(inv.get("total") or 0) == 0.0,
          str(dry)[:140])
    IDS["rate_calc"] = "rc_standard"
    IDS["principals"] = ",".join(p for p in principal_ids if p)
    mem_n = conn.execute(
        "SELECT count(*) c FROM iam_membership_v1 WHERE test_run_id=?",
        (NS,)).fetchone()["c"]
    IDS["memberships"] = f"count={mem_n}" if mem_n else ""
    ...
    # ---- 10b) SI4：归档后身份负例 —— 登录必须被拒绝，运营 IAM
    #      页面不显示，测试中心保留历史 ----
    denied = 0
    for short in ("pm", "fw", "sd", "an", "fin", "aud"):
        r = Session(f"post_{short}").login(f"{NS}_{short}", USER_PW)
        if not r.get("csrf_token"):
            denied += 1
    check("post_archive_login_rejected", denied == 6,
          f"denied={denied}/6")
    ops_principals = bill.get("/api/v1/iam/principals").get(
        "principals", [])
    check("post_archive_iam_page_clean",
          all(NS not in (p.get("username") or "")
              for p in ops_principals), f"visible={len(ops_principals)}")
    all_principals = bill.get(
        "/api/v1/iam/principals?include_fixture=true").get(
        "principals", [])
    check("post_archive_history_kept",
          any(NS in (p.get("username") or "") for p in all_principals),
          f"all={len(all_principals)}")
    ...
    try:
        ...
        abos_no_train = False
    report = {"protocol": "uatv6", "namespace": NS,
              "customer_id": CUST,
    ...
    REPORT.write_text(json.dumps(report, ensure_ascii=False, indent=2),
                      encoding="utf-8")
    print(f"\n{'✅ UAT V6 全链通过' if not failed else '❌ 存在失败'}"
